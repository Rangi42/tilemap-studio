...
public:
	...
	inline size_t top_row(void) const { return _tile2 ? std::min(_tile1->row(), _tile2->row()) : _tile1->row(); }
	inline size_t left_col(void) const { return _tile2 ? std::min(_tile1->col(), _tile2->col()) : _tile1->col(); }
